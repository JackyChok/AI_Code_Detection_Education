# Introduction
This code folder are forked from https://github.com/MacroChip/gpt-2-output-dataset/tree/master. 

We have added POST support for the Docker API.

Before running the Docker command, ensure that you have downloaded a pretrained detector model mentioned in the README from detector folder.

Then, ensure that the pretrained detector model is located at the same level as Dockerfile.

With Docker up and running, run the ```start.sh```
or use the following command
```
docker build <path to this folder> -t gpt2-detector
``` 
to create the detector model in Docker container.


API will be available on default http://localhost:8080. For the correct API address, refer to the output log from the Docker container.
